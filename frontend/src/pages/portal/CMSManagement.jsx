...
import {
  ...
  Clock, Phone, Mail, Check, Search, Loader2, FolderOpen, RefreshCw
} from 'lucide-react';
...
export default function CMSManagement() {
  ...
  const [drivePhotosCount, setDrivePhotosCount] = useState(() => getStoredDrivePhotos().length);
  const [driveSyncing, setDriveSyncing] = useState(false);
  const [driveSyncStatus, setDriveSyncStatus] = useState('');
  const [driveScriptUrl, setDriveScriptUrl] = useState(() => {
    return localStorage.getItem('bama_gdrive_script_url') || '';
  });

  const handleSyncDrive = async () => {
    setDriveSyncing(true);
    setDriveSyncStatus('Connecting to Google Drive folder...');
    try {
      if (driveScriptUrl) {
        localStorage.setItem('bama_gdrive_script_url', driveScriptUrl.trim());
        const updatedCfg = { ...cmsConfig, google_drive_script_url: driveScriptUrl.trim() };
        setCmsConfig(updatedCfg);
        await saveCmsConfig(updatedCfg);
      }
      const photos = await fetchGoogleDrivePhotos(true);
      setDrivePhotosCount(photos.length);
      setDriveSyncStatus(`✅ Successfully synced ${photos.length} photos from Google Drive!`);
    } catch (e) {
      setDriveSyncStatus('⚠️ Sync completed with local cache.');
    } finally {
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'GALLERY' && (
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-gray-200/90 shadow-sm space-y-5">
          ...
          {/* Google Drive Automatic Cloud Sync Panel */}
          <div className="bg-gradient-to-br from-amber-500/10 via-red-500/5 to-black/5 p-5 rounded-2xl border border-amber-500/30 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-amber-500 text-black flex items-center justify-center font-black shadow-md">
                  <FolderOpen className="w-5 h-5" />
                </div>
                <div>
                  <h4 className="text-sm font-black text-gray-900 flex items-center gap-2">
                    Official Google Drive Folder Sync
                  ...
                  <p className="text-xs text-gray-600 font-medium">
                    Folder ID: <span className="font-mono font-bold text-gray-900">{GOOGLE_DRIVE_FOLDER_ID}</span> • Synced Photos: <span className="font-bold text-amber-700">{drivePhotosCount} Photos</span>
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="px-3.5 py-2 bg-white hover:bg-gray-100 text-gray-800 border border-gray-300 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-xs transition"
                >
                ...
                <button
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-black font-black text-xs rounded-xl shadow-md flex items-center gap-1.5 transition cursor-pointer disabled:opacity-50"
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${driveSyncing ? 'animate-spin' : ''}`} />
                  <span>{driveSyncing ? 'Syncing...' : 'Sync Drive Photos'}</span>
                </button>
              </div>
            </div>

            <div className="pt-3 border-t border-amber-500/20 grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
              <div className="md:col-span-2">
                <label className="block text-[11px] font-bold text-gray-700 mb-1">
                  Google Apps Script Webhook URL (For 100% Automatic Cloud Sync):
                </label>
                <input
                  type="url"
                  ...
                  className="w-full bg-white border border-gray-300 rounded-xl px-3 py-2 text-xs font-mono text-gray-800 focus:outline-none focus:border-amber-500 shadow-xs"
                />
              </div>
              ...
              <div className="flex flex-col justify-end">
                <button
                  type="button"
                  onClick={handleSyncDrive}
                  className="w-full py-2 bg-gray-900 hover:bg-black text-white text-xs font-bold rounded-xl shadow-xs transition cursor-pointer"
                >
                  Save Webhook & Sync
                </button>
              </div>
            ...
            {driveSyncStatus && (
              <p className="text-xs font-bold text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-200">
                {driveSyncStatus}
              </p>
            )}
          </div>
